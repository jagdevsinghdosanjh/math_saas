...
def run_student() -> None:
    ...
    with tab_dashboard:
        try:
            render_dashboard()          # ← no args
        except Exception as exc:
            st.info(f"Dashboard coming soon. ({exc})")

    with tab_chapters:
        try:
            render_chapters_page()      # ← no args
        except Exception as exc:
    ...
    with tab_subs:
        try:
            render_subscriptions_page() # ← no args
        except Exception as exc:
            st.info(f"Subscription page coming soon. ({exc})")

    with tab_billing:
        try:
            render_billing_history()    # ← no args
        except Exception as exc:
            st.info(f"Billing page coming soon. ({exc})")

    with tab_public:
        try:
            render_public_content()     # ← no args
        except Exception as exc:
