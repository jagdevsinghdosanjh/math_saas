...
**Do not use `use_container_width`.** It is deprecated — Streamlit elements now stretch to fill their container by default. Use the `width` parameter instead: `width="stretch"` (equivalent to `width=stretch`) or `width="content"` (equivalent to `width=content`). Remove `use_container_width` when you see it, and never add it to new code.
