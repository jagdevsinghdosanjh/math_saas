...
def _call_ollama(model: str, prompt: str) -> str:
    """
    Low-level Ollama API call using /api/chat.
    Handles both streaming JSON and plain text responses.
    """
    ...
    try:
        ...
        try:
            data = response.json()
            print("RAW RESPONSE (JSON):", json.dumps(data, indent=2)[:500])
            ...
            # If not valid JSON, fall back to raw text
            ...
            # If nothing parsed, just return the raw text
            return raw_text
